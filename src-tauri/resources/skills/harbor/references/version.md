...
发布版本来自精确的 `v*` Git Tag，例如 `v0.2.0-preview.6` 对应应用版本 `0.2.0-preview.6`。Tag 后的开发构建会附加提交数量和短 SHA；Agent 仍应始终读取 `harbor --version`，不能从 Git 历史自行推测版本。
...
YAML 顶部的 `version` 是 **Harbor 应用版本**（与 `harbor --version` 相同，例如 `"0.2.0-preview.6"`）。
...
并将结果写入 YAML，例如 `version: "0.2.0-preview.6"`（建议加引号）。同一应用版本下无论改多少次 Task / Group，此值保持不变。
