...
2. 确认配置属于哪个项目，按 [taskcard/paths.md](taskcard/paths.md) 选择目录。
3. 项目配置必须检查当前 workspace 的 Search Paths 覆盖情况；未覆盖时由 Agent 按 [settings.md](settings.md) 直接维护当前 workspace 的 `search_paths`。
...
| [settings.md](settings.md) | 当前 workspace 的项目发现路径 |
| [web_api.md](web_api.md) | harbor_core 的第三方 HTTP 接口 |
