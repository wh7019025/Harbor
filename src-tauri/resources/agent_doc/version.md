...
YAML 顶部的 `version` 是 **Harbor 应用版本**（与 `harbor --version` 相同，例如 `"0.1.2-rc3"`）。
...
并将结果写入 YAML，例如 `version: "0.1.2-rc3"`（建议加引号）。同一应用版本下无论改多少次 Task / Group，此值保持不变。
